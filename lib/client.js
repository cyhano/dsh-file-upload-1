window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── 设置卡片样式：对齐官方 settings-plugins 的 PluginCard / ValueField 设计 ──
    // 全部走 --dsw-alias-* 设计 token，跟随深浅色主题；用 data-plugin-css 防重复注入。
    {
      const cssId = "dsh-file-upload/settings-card.css";
      if (typeof document !== "undefined" && document.querySelector('style[data-plugin-css="' + cssId + '"]') === null) {
        ...
        tag.textContent = [
          ".dfu-card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;list-style:none;transition:border-color .16s,background .16s}",
          ".dfu-card:hover{border-color:var(--dsw-alias-label-dimmed)}",
          ".dfu-cardOpen{background:var(--dsw-alias-bg-layer-2);border-color:var(--dsw-alias-label-dimmed)}",
          ".dfu-header{appearance:none;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:12px;align-items:center;gap:12px;padding:14px 16px;display:flex}",
          ".dfu-header:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:-2px}",
          ".dfu-headText{flex-direction:column;flex:1;gap:4px;min-width:0;display:flex}",
          ".dfu-name{color:var(--dsw-alias-label-primary);font-size:15px;font-weight:600;line-height:1.4}",
          ".dfu-desc{color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:1.5}",
          ".dfu-chevron{color:var(--dsw-alias-label-tertiary);flex:none;transition:transform .16s;display:inline-flex}",
          ".dfu-chevronOpen{transform:rotate(180deg)}",
          ".dfu-body{border-top:1px solid var(--dsw-alias-border-l2);margin:0 16px;padding-bottom:8px}",
          ".dfu-field{flex-direction:column;gap:6px;padding:12px 0;display:flex}",
          ".dfu-field+.dfu-field{border-top:1px solid var(--dsw-alias-border-l2)}",
          ".dfu-label{min-width:0;color:var(--dsw-alias-label-primary);flex:1;font-size:13px;font-weight:500;line-height:1.5}",
          ".dfu-input{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);height:34px;font:inherit;color:var(--dsw-alias-label-primary);border-radius:8px;padding:0 12px;font-size:13px;line-height:1.5;box-sizing:border-box;width:100%}",
          ".dfu-input:focus-visible{border-color:var(--dsw-alias-brand-primary);outline:none}",
          ".dfu-input:disabled{color:var(--dsw-alias-label-tertiary);cursor:default}",
          ".dfu-hint{color:var(--dsw-alias-label-tertiary);margin:0;font-size:12px;line-height:1.5}",
          ".dfu-footer{border-top:1px solid var(--dsw-alias-border-l2);justify-content:flex-end;align-items:center;gap:8px;padding:12px 0 4px;display:flex}",
          ".dfu-btn{appearance:none;font:inherit;cursor:pointer;border:1px solid #0000;border-radius:8px;padding:5px 14px;font-size:13px;line-height:1.5}",
          ".dfu-discard{border-color:var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);background:0 0}",
          ".dfu-discard:hover:not(:disabled){color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-label-dimmed)}",
          ".dfu-discard:disabled{opacity:.4;cursor:default}",
          ".dfu-save{background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3)}",
          ".dfu-save:disabled{opacity:.4;cursor:default}",
          ".dfu-save:focus-visible,.dfu-discard:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}",
        ].join("\n");
    ...
    /**
     * 设置页插件的卡片：可折叠，含「上传目录」「前缀文本」两个可编辑字段，
     * staged 编辑 + 保存/丢弃，视觉对齐官方 PluginCard / ValueField。
     */
    function UploadSettingsCard({ scope }) {
      const [open, setOpen] = react.useState(false);
      const [snapshot, setSnapshot] = react.useState(() => scope.getSnapshot());
      // staged 草稿（编辑先落本地，保存才写 namespace）
      const [dirDraft, setDirDraft] = react.useState("");
      const [prefixDraft, setPrefixDraft] = react.useState("");
      const [saving, setSaving] = react.useState(false);
      const [failed, setFailed] = react.useState(false);

      // 订阅 scope：首次与 scope 变更时回填草稿
      react.useEffect(() => {
        const sync = () => {
          const snap = scope.getSnapshot();
          setSnapshot(snap);
          setDirDraft(typeof snap.value?.uploadDir === "string" ? snap.value.uploadDir : "");
          setPrefixDraft(typeof snap.value?.prefix === "string" ? snap.value.prefix : "");
        };
        sync();
      ...
      const writable = snapshot.writable !== false && status === "ready";
      // 是否相对 base/默认有改动（粗判：与当前生效值不同即视为 dirty）
      const currentDir = typeof snapshot.value?.uploadDir === "string" ? snapshot.value.uploadDir : "";
      const currentPrefix = typeof snapshot.value?.prefix === "string" ? snapshot.value.prefix : "";
      const dirty = dirDraft !== currentDir || prefixDraft !== currentPrefix;
      ...
      const save = react.useCallback(() => {
        ...
        setFailed(false);
        Promise.all([
          scope.set("uploadDir", dirDraft),
          scope.set("prefix", prefixDraft),
        ])
          .then(() => { setFailed(false); })
          .catch(() => { setFailed(true); })
          .finally(() => setSaving(false));
      }, [scope, writable, saving, dirDraft, prefixDraft]);

      const discard = react.useCallback(() => {
        setDirDraft(currentDir);
        setPrefixDraft(currentPrefix);
        setFailed(false);
      }, [currentDir, currentPrefix]);

      // 不可用时整体不渲染（对齐官方 PluginCard 行为）
      if (status === "unavailable") return null;

      const field = (label, hint, value, onChange, placeholder) =>
        react.createElement("div", { className: "dfu-field" },
          react.createElement("label", { className: "dfu-label" }, label),
          ...
          react.createElement("p", { className: "dfu-hint" }, hint)
        );

      return react.createElement("div", {
        className: open ? "dfu-card dfu-cardOpen" : "dfu-card",
      },
        react.createElement("button", {
          type: "button",
          className: "dfu-header",
          "aria-expanded": open,
          onClick: () => setOpen(!open),
        },
          react.createElement("div", { className: "dfu-headText" },
            react.createElement("div", { className: "dfu-name" }, "文件上传"),
            react.createElement("div", { className: "dfu-desc" }, "上传文件的落盘目录与插入输入框的路径前缀")
          ),
          react.createElement("span", {
            className: open ? "dfu-chevron dfu-chevronOpen" : "dfu-chevron",
          }, react.createElement("svg", {
            width: 14, height: 14, viewBox: "0 0 16 16", fill: "none",
            stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round",
          }, react.createElement("path", { d: "M3.5 6L8 10.5L12.5 6" })))
        ),
        open ? react.createElement("div", { className: "dfu-body" },
          field("上传目录", "上传文件的落盘根目录；留空回退 /tmp。", dirDraft, setDirDraft, "/tmp"),
          field("前缀文本", "插入输入框时加在路径前的文本；留空表示不带前缀。", prefixDraft, setPrefixDraft, "[上传文件]"),
          react.createElement("div", { className: "dfu-footer" },
            failed ? react.createElement("p", { className: "dfu-hint", style: { flex: 1 } }, "保存失败，请重试") : null,
            react.createElement("button", {
              type: "button",
              className: "dfu-btn dfu-discard",
              disabled: !dirty || saving,
              onClick: discard,
            }, "丢弃"),
            react.createElement("button", {
              type: "button",
              className: "dfu-btn dfu-save",
